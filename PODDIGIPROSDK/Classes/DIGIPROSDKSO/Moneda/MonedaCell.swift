...
public class MonedaCell: Cell<String>, CellType, UITextFieldDelegate {
    ...
    public var genericRow: MonedaRow! {return row as? MonedaRow}
    ...
    open override func setup() {
        ...
        let apiObject = ObjectFormManager<MonedaCell>()
    ...
    func formatterCurrency(currency: String, textField: UITextField){
       let result = textField.text?.replacingOccurrences( of:"[^0-9.]", with: "", options: .regularExpression)
        let numberFormatter = NumberFormatter()
        numberFormatter.locale = Locale(identifier: currency)
        numberFormatter.numberStyle = .currency
        numberFormatter.minimumFractionDigits = 2
        numberFormatter.maximumFractionDigits = 2
        let amount:String? = result
        if let amountValue = amount, let am = Float(amountValue) {
            let resultFormat = numberFormatter.string(from: am as NSNumber)
            print("FORMAT: \(String(describing: resultFormat))")
    ...
    open func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        case "es":
            self.formatterCurrency(currency: "mxn_Mx", textField: textField)
            break
        case "en":
            self.formatterCurrency(currency: "en_US", textField: textField)
            break
        case "ja":
            self.formatterCurrency(currency: "ja_JP", textField: textField)
            break
        case "cs":
            self.formatterCurrency(currency: "cs_CZ", textField: textField)
            break
        case "en-gb":
            self.formatterCurrency(currency: "en_UK", textField: textField)
            break
        case "fr":
            self.formatterCurrency(currency: "es_ES", textField: textField)
            break
        case "th":
            self.formatterCurrency(currency: "th_TH", textField: textField)
            break
        case "pt-br":
            self.formatterCurrency(currency: "pt_BR", textField: textField)
            break
        case "ru":
            self.formatterCurrency(currency: "ru_RU", textField: textField)
            break
        default:
            self.formatterCurrency(currency: "mxn_Mx", textField: textField)
